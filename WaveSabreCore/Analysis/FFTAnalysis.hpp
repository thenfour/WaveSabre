...
public:
  ...
  const std::vector<SpectrumBin>& GetSpectrum() const
  {
    return mSpectrum;
  }
  ...
  {
    return mSampleRate / mFFTSizeInt;
  }
...
public:
  ...
  WindowType GetWindowType() const
  {
    return mAnalyzers[0].GetWindowType();
  }
  FFTSize GetFFTSize() const
  {
    return mAnalyzers[0].GetFFTSize();
  }
  int GetFFTSizeInt() const
  {
    return mAnalyzers[0].GetFFTSizeInt();
  }
  float GetSampleRate() const
  {
    return mAnalyzers[0].GetSampleRate();
  }
  int GetOverlapFactor() const
  {
...
private:
  ...
  int InactiveBufferIndex() const
  {
    return 1 - mActiveBuffer.load(std::memory_order_acquire);
  }
  void PublishBuffer(int idx)
  {
    mActiveBuffer.store(idx, std::memory_order_release);
    mHasNewOutput.store(true, std::memory_order_release);
  }
...
public:
  ...
  void SetSampleRate(float sampleRate)
  {
    mFFTAnalysis.SetSampleRate(sampleRate);
  ...
  void SetOverlapFactor(int factor)
  {
    mFFTAnalysis.SetOverlapFactor(factor);
    SetFFTUpdateRate(mFFTAnalysis.GetFFTSizeInt(), factor);
  }
  void SetInputDecimationFactor(int factor);
  // Proxy getters for UI
  MonoFFTAnalysis::WindowType GetWindowType() const
  {
    return mFFTAnalysis.GetWindowType();
  }
  MonoFFTAnalysis::FFTSize GetFFTSize() const
  {
    return mFFTAnalysis.GetFFTSize();
  }
  int GetFFTSizeInt() const
  {
  ...
  int GetOverlapFactor() const
  {
    return mFFTAnalysis.GetOverlapFactor();
  }
  int GetInputDecimationFactor() const
  {
    return mInputDecimationFactor;
  }
  ...
  {
    return mCurrentHoldTimeMs;
  }
  float GetFalloffTime() const
  {
    return mCurrentFalloffTimeMs;
  }
  float GetAveragingWindow() const
  {
    return mCurrentAveragingWindowMs;
  }
  ...
  const std::vector<SpectrumBin>& GetSpectrum() const
  {
    return mBuffers[0][mActiveBuffer.load(std::memory_order_acquire)];
  }
  float GetMagnitudeAtFrequency(float frequency) const override;
  float GetFrequencyResolution() const override
  {
    return mFFTAnalysis.GetFrequencyResolution();
  }
  float GetNyquistFrequency() const override
  {
    return mFFTAnalysis.GetNyquistFrequency();
  }
  const IFrequencyAnalysis& GetLeftView() const
  {
    return mLeftView;
  }
  const IFrequencyAnalysis& GetRightView() const
  {
    return mRightView;
  }
  ...
  bool HasNewOutput() const
  {
    return mHasNewOutput.load(std::memory_order_acquire);
  }
  void ConsumeOutput()
  {
...
public:
  ...
  void SetFFTUpdateRate(int fftSize, int overlapFactor)
  {
