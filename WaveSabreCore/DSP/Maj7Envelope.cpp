...
namespace M7
{
EnvelopeNode::EnvelopeNode(ModMatrixNode& modMatrix, real_t* paramCache, const EnvelopeInfo& envInfo)
    :  //ModDestination modDestIDDelayTime, , int paramBaseID, ModSource myModSource) :
...
void EnvelopeNode::EnvelopeNoteOff()
{
  ...
  if (mMode == EnvelopeMode::OneShot)
    return;  // in one-shot there's no way to cut off the note faster.
...
void EnvelopeNode::RecalcState()
{
  ...
  {
    auto ms = mParams.GetPowCurvedValue(paramOffset,
                                        gEnvTimeCfg,
                                        mModMatrix.GetDestinationValue(mModDestBase + (int)modid));
