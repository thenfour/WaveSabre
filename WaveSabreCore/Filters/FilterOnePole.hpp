...
public:
  ...
  virtual std::unique_ptr<IFilter> Clone() const override
  {
    auto clone = std::make_unique<VanillaOnePoleFilter>();
    clone->mResponse = this->mResponse;
    clone->m_cutoffHz = this->m_cutoffHz;
    clone->m_a0 = this->m_a0;
    clone->m_b1 = this->m_b1;
    clone->m_z_1 = this->m_z_1;
    return clone;
  }
  ...
  virtual real GetMagnitudeAtFrequency(real freqHz) const override
  {
    ...
    const real w = math::gPITimes2 * freqHz * Helpers::CurrentSampleRateRecipF;
    const real c = math::cos(w);
    const real s = math::sin(w);

    // H_lp(z) = a0 / (1 - b1 z^-1), with z = e^(jw)
    // Denominator = (1 - b1 cos w) + j (b1 sin w)
    const real A = (real)1 - m_b1 * c;
    const real B = m_b1 * s;
    const real D = A * A + B * B;
    ...
    switch (mResponse)
    {
      default:
      case FilterResponse::Lowpass:
        return std::abs(m_a0) / math::sqrt(D);

      case FilterResponse::Highpass:
      {
        // HP is implemented as: H_hp = 1 - H_lp
        const real lpRe = m_a0 * A / D;
        const real lpIm = -m_a0 * B / D;

        const real hpRe = (real)1 - lpRe;
        const real hpIm = -lpIm;

        return math::sqrt(hpRe * hpRe + hpIm * hpIm);
      }
    }
  }
