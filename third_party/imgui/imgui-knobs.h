...
struct KnobColorMod
{
  ...
  KnobColorMod(const char* inactiveTrackColor,
               const char* activeTrackColor,
  ...
  explicit KnobColorMod(const char* htmlColor, bool enabled = true)
  {
    mActiveTrackColor = WaveSabreVstLib::ColorFromHTML(htmlColor, enabled ? 0.5f : 0.3f);
    mActiveTrackHoverColor = WaveSabreVstLib::ColorFromHTML(htmlColor, enabled ? 0.5f : 0.5f);
    mInactiveTrackColor = WaveSabreVstLib::ColorFromHTML("666666", enabled ? 0.5f : 0.5f);
    mInactiveTrackHoverColor = WaveSabreVstLib::ColorFromHTML("666666", enabled ? 0.5f : 0.5f);
    mThumbColor = WaveSabreVstLib::ColorFromHTML(htmlColor, enabled ? 1.f : 0.5f);
    mThumbHoverColor = WaveSabreVstLib::ColorFromHTML(htmlColor, enabled ? 1.f : 0.7f);
    mTextColor = WaveSabreVstLib::ColorFromHTML(enabled ? "ffffff" : "666666");
  }
};
...
struct ColorMod
{
  ...
  static ImColor GetModifiedColor(ImVec4 orig, float mHueAdd, float mSaturationMul, float mValueMul)
  {
    ...
    auto newcolor = ImColor::HSV(WaveSabreCore::M7::math::fract(h + mHueAdd),
                                 WaveSabreCore::M7::math::clamp01(s * mSaturationMul),
                                 WaveSabreCore::M7::math::clamp01(v * mValueMul));
    return newcolor;
  }
...
inline static void AddCurveToPath(ImDrawList* dl,
                                  ImVec2 pos,
                                  ImVec2 size,
                                  bool invertX,
                                  bool invertY,
...
bool KnobWithEditableLabel(std::string& label,
                           const char* id,
                           float* p_value,
                           float v_min,
                           float v_max,
                           ...
                           int steps = 10,
                           IValueConverter* conv = nullptr,
                           void* capture = nullptr);
bool Knob(const char* labelAndID,
          float* p_value,
          float v_min,
          float v_max,
          float v_default,
          ...
          IValueConverter* conv = nullptr,
          void* capture = nullptr);
bool KnobInt(const char* labelAndID,
             int* p_value,
             int v_min,
             int v_max,
             int v_default,
