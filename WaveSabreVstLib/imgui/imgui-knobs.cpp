...
namespace ImGuiKnobs {
    ...
    template<typename DataType>
    bool BaseKnob(const char* label, ImGuiDataType data_type, DataType* p_value, DataType v_min, DataType v_max, DataType v_default, float speed, const char* format, ImGuiKnobVariant variant, float size, ImGuiKnobFlags flags, int steps, IValueConverter* conv, void* capture) {
        auto knob = detail::knob_with_drag(label, data_type, p_value, v_min, v_max, v_default, speed, format, size, flags, conv, capture);
        ...
        case ImGuiKnobVariant_WiperOnly: {
            ...
            if (knob.t > 0.01) {
                knob.draw_arc(0.8f, 0.9f, knob.angle_min, knob.angle, detail::GetPrimaryColorSet(), 16, 2);
            }
    ...
    bool Knob(const char* label, float* p_value, float v_min, float v_max, float v_default, float normalSpeed, float slowSpeed,
    ...
    bool KnobInt(const char* label, int* p_value, int v_min, int v_max, int v_default, float normalSpeed, float slowSpeed,
        const char* format, ImGuiKnobVariant variant, float size, ImGuiKnobFlags flags, int steps, IValueConverter* conv, void* capture)
    {
        const char* _format = format == NULL ? "%i" : format;
        return BaseKnob(label, ImGuiDataType_S32, p_value, v_min, v_max, v_default, ImGui::GetIO().KeyShift ? slowSpeed : normalSpeed, _format, variant, size, flags, steps, conv, capture);
    }
