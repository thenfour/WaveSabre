...
{
  if (value.mNumericValue.Get<int>() < 0)
    return (Tenum)0;
  if (value.mNumericValue.Get<int>() >= (int)count)
    return (Tenum)0;
...
inline bool VisitChildren(clarinoid::JsonVariantReader& parent,
                          std::function<void(clarinoid::JsonVariantReader&)> handler)
{
  while (true)
  {
...
inline int SetSimpleJSONVstChunk(TDevice* pDevice,
                                 ...
                                 std::function<void(clarinoid::JsonVariantReader&)> setVSTParam)
{
  ...
  {
    std::map<std::string, std::pair<bool, size_t>>
        paramMap;  // maps string name to whether it's been set + which param index is it.
    for (size_t i = 0; i < paramCount; ++i)
    {
      paramMap[paramNames[i]] = std::make_pair(false, i);
    }

    if (!VisitChildren(paramsObj,
...
template <size_t paramCount>
inline int GetSimpleJSONVstChunk(const std::string& keyName,
                                 void** data,
...
inline void CopyPatchToClipboard(
    HWND hWnd,
    VstPlug*
        p /*, const std::string& keyName, float const (&paramCache)[paramCount], char const* const (&paramNames)[paramCount]*/)
{
  void* data = nullptr;
...
template <size_t paramCount>
void CopyParamCache(HWND hWnd,
                    const std::string& symbolName,
                    const std::string& paramCountExpr,
                    float const (&paramCache)[paramCount],
                    char const* const (&paramNames)[paramCount])
{
  std::stringstream ss;

  ss << "static_assert((int)" << paramCountExpr << " == " << paramCount
...
inline void PopulateStandardMenuBar(HWND hWnd,
                                    const std::string& vstName,
                                    TDevice* pDevice,
                                    VstPlug* pVst,
                                    const std::string& defaultsArrayName,
                                    const std::string& paramCountExpr,
                                    float const (&paramCache)[paramCount],
                                    char const* const (&paramNames)[paramCount])
{
  ...
  if (ImGui::BeginMenu("Debug"))
  {
    ...
    if (ImGui::MenuItem("Export as C++ defaults to clipboard"))
    {
      if (IDYES == ::MessageBoxA(hWnd,
                                 "Code will be copied to the clipboard, based on 1st item params.",
                                 vstName.c_str(),
    ...
    if (ImGui::MenuItem("Optimize"))
    {
      ...
      {
        if (IDYES ==
            ::MessageBoxA(hWnd, "Backup current patch to clipboard?", "WaveSabre - Maj7", MB_YESNO | MB_ICONQUESTION))
        {
          CopyPatchToClipboard(hWnd, pVst);
    ...
    {
      if (IDYES == ::MessageBoxA(hWnd,
                                 "Make sure you save first. If there's any problem, it will ruin your patch. This "
                                 "optimizes & serializes the patch in the wavesabre format, then deserializes it, and "
                                 "compares the before/after param cache for errors.",
                                 "WaveSabre - Maj7",
                                 MB_YESNO | MB_ICONQUESTION))
      {
        if (IDYES ==
