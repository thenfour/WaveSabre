...
namespace clarinoid
{
    ...
    //inline std::string EncodeBase64(const uint8_t* data, size_t size) {
    //    std::string base64_chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
    //    std::string encoded_string;
    //    size_t i = 0;

    //    while (size > 2) {
    //        encoded_string += base64_chars[(data[i] & 0xFC) >> 2];
    //        encoded_string += base64_chars[((data[i] & 0x03) << 4) + ((data[i + 1] & 0xF0) >> 4)];
    //        encoded_string += base64_chars[((data[i + 1] & 0x0F) << 2) + ((data[i + 2] & 0xC0) >> 6)];
    //        encoded_string += base64_chars[data[i + 2] & 0x3F];
    ...
    //    if (size == 1) {
    //        encoded_string += base64_chars[(data[i] & 0xFC) >> 2];
    //        encoded_string += base64_chars[(data[i] & 0x03) << 4];
    //        encoded_string += "==";
    //    }
    //    else if (size == 2) {
    //        encoded_string += base64_chars[(data[i] & 0xFC) >> 2];
    //        encoded_string += base64_chars[((data[i] & 0x03) << 4) + ((data[i + 1] & 0xF0) >> 4)];
    //        encoded_string += base64_chars[(data[i + 1] & 0x0F) << 2];
    //        encoded_string += "=";
    //    }
    ...
    //    // Build the base64 decoding table
    ...
    //            n <<= 6;
    //            if (i + j < input.size()) {
    //                size_t pos = table.find(input[i + j]);
    //                if (pos != std::string::npos) {
    //                    n |= pos;
    //                }
    //                else {
    //                    return {};// throw std::runtime_error("Invalid character in input");
    //                }
    //            }
    //            else {
    //                // Padding character
    ...
    static inline std::vector<BYTE> base64_decode(std::string const& encoded_string) {
...
struct MemoryStream : IStream
{
    ...
    MemoryStream() {}
    explicit MemoryStream(const uint8_t* buf, size_t size)
    {
        write(buf, size);
        mCursor = 0;
    }
    ...
    {
        mBuffer.reserve(mBuffer.size() + bytes);
    ...
    {
        for (size_t i = 0; i < bytes; ++i) {
            if (mCursor >= mBuffer.size()) return i;
            buf[i] = mBuffer[mCursor];
            ++mCursor;
        }
        return bytes;
...
namespace CCJSON
{
    ...
    static constexpr bool gQuoteKeyNames = true;

template <typename T, typename F_>
inline T alias_cast(F_ raw_data)
{
    union {
        F_ raw;
        T data;
...
struct EscapeSequence
{
    ...
    static inline char escapeChar(char c)
    {
    ...
    static inline char unescapeChar(char c)
    {
  ...
  private:
    static inline const char *escapeTable(bool excludeSolidus)
    {
...
struct FloatTraits<T, sizeof(double)>
{
    ...
    {
        static uint32_t const factors[18] PROGMEM = {
            0x3FB99999,
            0x9999999A, // 1e-1
            0x3F847AE1,
            0x47AE147B, // 1e-2
...
inline String UnsignedIntegerToStringWithOptionalPrefix(UInt value, char prefixChar = 0)
{
    char buffer[24] = {0};
...
inline String UnsignedIntegerToString(UInt value)
{
    return UnsignedIntegerToStringWithOptionalPrefix(value, 0);
}

inline String SignedIntegerToString(Integer value)
{
    String ret;
    if (value < 0)
    {
...
struct JsonVariantWriter : JsonParentChildHelper
{
    ...
    {
        return String("[") + std::to_string(mStream.mLine) + ":" + std::to_string(mStream.mColumn) + "]";
    }
...
struct JsonVariantReader : JsonParentChildHelper
{
    ...
    Result ParseQuotedString(String &stringStorage)
    {
        ...
        for (;;)
        {
            ...
            stringStorage.push_back((char)c);
        }
...
// size-optimizing
inline Result Deserialize(JsonVariantReader &myval, SerializationMapping *begin, SerializationMapping *end)
{
...
// size-optimizing
inline Result Serialize(JsonVariantWriter &myval, SerializationMapping *begin, SerializationMapping *end)
{
    Result ret = Result::Success();
    myval.BeginObject();
    for (auto i = begin; i != end; ++i)
...
inline SerializationMapping CreateSerializationMapping(Tparam &param, const char *key)
{
    return
    {
        &param,
...
template <size_t N>
inline Result Serialize(JsonVariantWriter &myval, SerializationObjectMap<N> &map)
{
    return map.Serialize(myval);
}

template <size_t N>
inline Result Deserialize(JsonVariantReader &myval, SerializationObjectMap<N> &map)
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
inline Result Serialize(JsonVariantWriter &myval,
                 intptr_t begin,
                 intptr_t end,
                 size_t tsize,
...
template <typename T, size_t N>
inline Result Serialize(JsonVariantWriter &myval, std::array<T, N> &param)
{
...
inline Result Deserialize(JsonVariantReader &myval,
                   intptr_t begin,
                   intptr_t end,
                   size_t tsize,
...
template <typename T, size_t N>
inline Result Deserialize(JsonVariantReader &myval, std::array<T, N> &param)
{
...
inline Result Serialize(JsonVariantWriter &myval, bool &param)
{
    return myval.WriteBoolean(param);
}

inline Result Deserialize(JsonVariantReader &myval, bool &param)
{
...
// any serializable object provide these 2 overloads
template <typename T, std::enable_if_t<std::is_floating_point<T>::value || std::is_integral<T>::value, int> = 0>
inline Result Serialize(JsonVariantWriter &myval, T &param)
{
    return myval.WriteNumberValue(param);
}

template <typename T, std::enable_if_t<std::is_floating_point<T>::value || std::is_integral<T>::value, int> = 0>
inline Result Deserialize(JsonVariantReader &myval, T &param)
{
