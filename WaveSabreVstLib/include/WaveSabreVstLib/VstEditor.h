...
struct PowCurvedConverter : ImGuiKnobs::IValueConverter {
  ...
  virtual std::string ParamToDisplayString(double param, void *capture,
                                           bool inputContinuity) override {
  ...
  virtual std::pair<ImGuiKnobs::variant, bool>
  DisplayValueToParam(const std::string &s, void *capture) override {
    try {
      double d = std::stod(s);
      M7::QuickParam p;
...
struct M7VolumeConverter : ImGuiKnobs::IValueConverter {
  ...
  M7VolumeConverter(const M7::VolumeParamConfig &cfg) : mCfg(cfg) {}
...
struct HistoryViewSeries {
  // HistoryEnvFollower mFollower; // actual sample values are not helpful; we
  // need a peak env follower to look nice.
...
template <size_t TSeriesCount, int Twidth, int Theight> struct HistoryView {
  ...
  void Render(const std::array<HistoryViewSeriesConfig, TSeriesCount> &cfg,
    ...
    for (size_t i = 0; i < TSeriesCount; ++i) {
      ...
      // float lin =
      // mSeries[i].mFollower.ProcessSample(M7::math::DecibelsToLinear(dbValues[i]));
...
static inline std::vector<std::pair<std::string, int>> LoadGmDlsOptions() {
  ...
  // optionNames.insert(pair<int, string>(noSampleOption.second,
  // noSampleOption.first));
...
inline void CompressorTransferCurve(const MonoCompressor &c, ImVec2 size,
  ...
  for (int iSeg = 0; iSeg < segmentCount; ++iSeg) {
    ...
    float dbIn =
        (1.0f - inLin) * gHistoryViewMinDB; // db from gHistoryViewMinDB to 0.
  ...
  double detectorLevel = std::max(
      detectorAnalysis[0].mCurrentPeak,
      detectorAnalysis[1]
          .mCurrentPeak); // std::max(mpMaj7Comp->mComp[0].mPostDetector,
                          // mpMaj7Comp->mComp[1].mPostDetector);
  ...
  if (detAtten > 0.00001) {
    dl->AddCircleFilled({dbToX(detectorDB), dbToY(detOutDB)}, 6.0f,
                        detectorColorAtt);
  } else {
    dl->AddCircleFilled({dbToX(detectorDB), dbToY(detOutDB)}, 3.0f,
                        detectorColor);
  }
...
inline void RenderTransferCurve(ImVec2 size, const TransferCurveColors &colors,
                                std::function<float(float)> transferFn) {
  auto *dl = ImGui::GetWindowDrawList();
  ImRect bb;
  bb.Min = ImGui::GetCursorScreenPos();
  bb.Max = bb.Min + size;
  ...
  dl->AddPolyline(clipPoints.data(), (int)clipPoints.size(), colors.lineClip, 0,
                  3);
...
inline bool VUMeter(const char *id, const double *rmsLevel,
  ...
  auto RenderDBRegion = [&](float dB1, float dB2, ImColor bgUnderUnity,
                            ImColor bgOverUnity) {
    if (dB1 > dB2)
    ...
    if (cmax <=
        0) { // Entire range is negative; render cmin,cmax in under-unity style
      dl->AddRectFilled({bb.Min.x, DbToY(cmin)}, {bb.Max.x, DbToY(cmax)},
                        bgUnderUnity);
    } else if (cmin >= 0) { // Entire range is non-negative, render cmin,cmax in
                            // over-unity style
      dl->AddRectFilled({bb.Min.x, DbToY(cmin)}, {bb.Max.x, DbToY(cmax)},
                        bgOverUnity);
    } else {
      // Range spans across 0, split into two regions
      dl->AddRectFilled({bb.Min.x, DbToY(cmin)}, {bb.Max.x, DbToY(0)},
                        bgUnderUnity);
      dl->AddRectFilled({bb.Min.x, DbToY(0)}, {bb.Max.x, DbToY(cmax)},
                        bgOverUnity);
    }
  };

  RenderDBRegion(cfg.minDB, cfg.maxDB, colors.background,
                 colors.backgroundOverUnity);
  ...
  if (peakLevel) {
    ...
    if (cfg.levelMode == VUMeterLevelMode::Attenuation)
      fgBaseDB = 0;
    RenderDBRegion(peakDb, fgBaseDB, colors.foregroundPeak,
                   colors.foregroundOverUnity);
  }
  ...
  if (rmsLevel) {
    ...
    if (cfg.levelMode == VUMeterLevelMode::Attenuation)
      fgBaseDB = 0;
    RenderDBRegion(rmsDB, fgBaseDB, colors.foregroundRMS,
                   colors.foregroundOverUnity);
  }
  ...
  // draw clip
  bool allowClipIndicator =
      true; // (cfg.levelMode != VUMeterLevelMode::Attenuation);
...
inline void VUMeter(const char *id, AnalysisStream &a0, AnalysisStream &a1,
  ...
  static const std::vector<VUMeterTick> smallTickSet = {
      {-10.0f, "10db"}, {-20.0f, nullptr}, {-30.0f, "30db"}, {-40.0f, nullptr},
      //{-50.0f, "50db"},
  };

  const VUMeterConfig cfg = {
      size,        VUMeterLevelMode::Audio, VUMeterUnits::Linear, -50, 6,
      smallTickSet};
...
inline void VUMeterAtten(const char *id, IAnalysisStream &a0,
                         IAnalysisStream &a1, ImVec2 size = {30, 300}) {
  static const std::vector<VUMeterTick> smallTickSet = {
      {-1, nullptr}, {-2, nullptr}, {-3, "-3"},
      {-6, "-6"},    {-9, "-9"},    {-12, "-12"},
  };

  const VUMeterConfig cfg = {size,
  ...
  ImGui::PushID(id);
  if (VUMeter("VU L", nullptr, &a0.mCurrentPeak, &a0.mCurrentHeldPeak,
              &a0.mClipIndicator, true, cfg)) {
    a0.Reset();
    a1.Reset();
  }
  ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, {1, 0});
  ImGui::SameLine();
  if (VUMeter("VU R", nullptr, &a1.mCurrentPeak, &a1.mCurrentHeldPeak,
              &a1.mClipIndicator, false, cfg)) {
    a0.Reset();
    a1.Reset();
  }
  ImGui::PopID();
...
public:
  ...
  bool BeginTabBar2(const char *str_id, ImGuiTabBarFlags flags,
                    float width = 0) {
  ...
  bool WSBeginTabItem(const char *label, bool *p_open = 0,
  ...
  // selected tab.
  bool WSBeginTabItemWithSel(const char *label, int thisTabIndex,
  ...
  void EndTabBarWithColoredSeparator() {
    ...
    // float separator_max_x = cursor.x;//
    // +M7::math::floor(window->WindowPadding.x * 0.5f);
  ...
  struct ScaledFloatConverter : ImGuiKnobs::IValueConverter {
    ...
    ScaledFloatConverter(float v_min, float v_max) : mMin(v_min), mMax(v_max) {}
  ...
  struct Maj7IntConverter : ImGuiKnobs::IValueConverter {
    ...
    Maj7IntConverter(const M7::IntParamConfig &cfg) : mCfg(cfg) {}
  ...
  struct Maj7FrequencyConverter : ImGuiKnobs::IValueConverter {
    ...
    Maj7FrequencyConverter(M7::FreqParamConfig cfg,
                           VstInt32 ktParamID /*pass -1 if no KT*/)
        : mCfg(cfg), mKTParamID(ktParamID) {}

    // returns whether keytracking is on
    bool TouchKeytracking(void *capture) {
      mBacking[1] = 0;
      if (mKTParamID < 0) {
        return false;
      }
    ...
    // when KT is off, the value is in hz
    // when KT is on, the value is in 0-1
    virtual std::pair<ImGuiKnobs::variant, bool>
    DisplayValueToParam(const std::string &s, void *capture) override {
      try {
        if (TouchKeytracking(capture)) {
  ...
  bool WSImGuiParamKnob(VstInt32 id, const char *name,
    ...
    switch (behavior) {
      ...
      //{
      //	int iparam =
      //::WaveSabreCore::Helpers::ParamToUnisono(paramValue); 	r =
      // ImGuiKnobs::KnobInt(name, &iparam, 1, 16, 1, 0, .1f, .001f, NULL,
      // ImGuiKnobVariant_WiperOnly); 	if (r) { 		paramValue =
      //::WaveSabreCore::Helpers::UnisonoToParam(iparam);
      //		GetEffectX()->setParameterAutomated(id,
      // M7::math::clamp01(paramValue));
      //	}
      //	break;
      // }
    ...
    default: {
      r = ImGuiKnobs::Knob(name, &paramValue, 0, 1, 0.5f, 0,
                           ImGuiKnobs::ModInfo{}, 0.003f, 0.0001f, fmt,
  ...
  void Maj7ImGuiBoolParamToggleButton(TParamID paramID, const char *label,
    ...
    ImGui::PopStyleVar(
        2); // ImGuiStyleVar_ItemSpacing & ImGuiStyleVar_FrameRounding
  }
  ...
  void WSImGuiParamEnumList(VstInt32 paramID, const char *ctrlLabel,
    ...
    enum AnyEnum {};
    auto friendlyVal = pa.GetEnumValue<AnyEnum>(
        0); // ParamToEnum(paramValue, elementCount);
    ...
    if (ImGui::BeginListBox("##enumlist",
                            CalcListBoxSize(0.2f + elementCount))) {
      for (int n = 0; n < elementCount; n++) {
        const bool is_selected = (enumIndex == n);
        if (ImGui::Selectable(captions[n], is_selected)) {
          enumIndex = n;
          pa.SetEnumIntValue(0, n);
          // paramValue = EnumToParam(n, elementCount);//
          // ::WaveSabreCore::Helpers::StateVariableFilterTypeToParam((::WaveSabreCore::StateVariableFilterType)enumIndex);
  ...
  void Maj7ImGuiParamEnumList(TparamID paramID, const char *ctrlLabel,
                              Tcount elementCount, Tenum defaultVal,
                              const char *const *const captions) {
    // M7::real_t tempVal;
    ...
    auto friendlyVal = p.GetEnumValue<
        Tenum>(); // ParamToEnum(paramValue, elementCount);
                  // //::WaveSabreCore::Helpers::ParamToStateVariableFilterType(paramValue);
    int enumIndex = (int)friendlyVal;
    ...
    if (ImGui::BeginListBox("##enumlist",
                            CalcListBoxSize(0.2f + (int)elementCount))) {
      for (int n = 0; n < (int)elementCount; n++) {
        const bool is_selected = (enumIndex == n);
        if (ImGui::Selectable(captions[n], is_selected)) {
          p.SetEnumValue((Tenum)n);
          GetEffectX()->setParameterAutomated((VstInt32)paramID,
                                              p.GetRawValue());
        }
  ...
  bool ComboLookingButton(const char *label, const char *preview_value) {
    ...
    // Render shape
    const ImU32 frame_col = ImGui::GetColorU32(hovered ? ImGuiCol_FrameBgHovered
                                                       : ImGuiCol_FrameBg);
    ...
    if (label_size.x > 0)
      ImGui::RenderText(ImVec2(bb.Max.x + style.ItemInnerSpacing.x,
                               bb.Min.y + style.FramePadding.y),
                        label);
  ...
  template <typename Tenum, typename TparamID, typename Tcount>
  void Maj7ImGuiParamEnumCombo(TparamID paramID, const char *ctrlLabel,
                               Tcount elementCount, Tenum defaultVal,
                               const char *const *const captions,
                               float width = 120) {
    // M7::real_t tempVal;
    ...
    if (ImGui::BeginPopup("##enumlist")) {
      ...
      ImGui::InputText("##filterText", mFilterInputText,
                       std::size(mFilterInputText));
      ...
      auto matches = (std::strlen(mFilterInputText) > 0)
                         ? autocomplete(mFilterInputText, options)
                         : options;
  ...
  void Maj7ImGuiParamEnumToggleButtonArray(
      TparamID paramID, const char *ctrlLabel, float width,
      ...
      if (is_selected) {
        if (cfg.selectedColor) {
          ImGui::PushStyleColor(ImGuiCol_Button,
                                (ImVec4)ColorFromHTML(cfg.selectedColor));
        ...
        if (cfg.selectedHoveredColor) {
          ImGui::PushStyleColor(
              ...
              (ImVec4)ColorFromHTML(cfg.selectedHoveredColor));
          colorsPushed++;
        }
      } else {
        if (cfg.notSelectedColor) {
          ImGui::PushStyleColor(ImGuiCol_Button,
                                (ImVec4)ColorFromHTML(cfg.notSelectedColor));
          ImGui::PushStyleColor(ImGuiCol_ButtonActive,
                                (ImVec4)ColorFromHTML(cfg.notSelectedColor));
          colorsPushed += 2;
        }
        if (cfg.notSelectedHoveredColor) {
          ImGui::PushStyleColor(
              ImGuiCol_ButtonHovered,
              (ImVec4)ColorFromHTML(cfg.notSelectedHoveredColor));
          colorsPushed++;
        }
      }
  ...
  void Maj7ImGuiParamEnumMutexButtonArray(
      TparamID paramID, const char *ctrlLabel, float width, bool horiz,
      const EnumMutexButtonArrayItem<Tenum> (&itemCfg)[Tcount], int spacing = 0,
      int columns = 0) {
    ...
    auto coalesce = [](const char *a, const char *b) { return !!a ? a : b; };
    ...
    for (size_t i = 0; i < Tcount; i++) {
      ...
      if (is_selected) {
        ImGui::PushStyleColor(ImGuiCol_Button,
                              (ImVec4)ColorFromHTML(coalesce(
                                  cfg.selectedColor, defaultSelectedColor)));
        ImGui::PushStyleColor(ImGuiCol_ButtonActive,
                              (ImVec4)ColorFromHTML(coalesce(
                                  cfg.selectedColor, defaultSelectedColor)));
        colorsPushed += 2;
      } else {
        ImGui::PushStyleColor(ImGuiCol_Button, (ImVec4)ColorFromHTML(coalesce(
                                                   cfg.notSelectedColor,
                                                   defaultNotSelectedColor)));
        ImGui::PushStyleColor(
            ImGuiCol_ButtonActive,
            (ImVec4)ColorFromHTML(
                coalesce(cfg.notSelectedColor, defaultNotSelectedColor)));
        ImGui::PushStyleColor(
            ImGuiCol_ButtonHovered,
            (ImVec4)ColorFromHTML(coalesce(cfg.notSelectedHoveredColor,
  ...
  void Maj7ImGuiParamBoolToggleButtonArray(
      const char *ctrlLabel, float width,
  ...
  void Maj7ImGuiParamScaledFloat(VstInt32 paramID, const char *label,
                                 M7::real_t v_min, M7::real_t v_max,
                                 M7::real_t v_defaultScaled,
                                 float v_centerScaled, float sizePixels,
    ...
    if (ImGuiKnobs::Knob(label, &tempVal, 0, 1, defaultParamVal, centerParamVal,
                         ...
                         ImGuiKnobVariant_WiperOnly, sizePixels,
                         ImGuiKnobFlags_CustomInput, 10, &conv, this)) {
  ...
  void Maj7ImGuiParamFMKnob(VstInt32 paramID, const char *label) {
    WaveSabreCore::M7::real_t tempVal =
        GetEffectX()->getParameter((VstInt32)paramID);
    M7::ParamAccessor pa{&tempVal, 0};
    const float v_default = 0;
    FMValueConverter conv;
    if (ImGuiKnobs::Knob(label, &tempVal, 0, 1, v_default, 0,
                         ImGuiKnobs::ModInfo{}, gNormalKnobSpeed,
                         gSlowKnobSpeed, "%.2f",
                         ImGuiKnobVariant_ProgressBarWithValue, 0,
  ...
  void Maj7ImGuiParamInt(VstInt32 paramID, const char *label,
                         const M7::IntParamConfig &cfg, int v_defaultScaled,
                         int v_centerScaled) {
    WaveSabreCore::M7::real_t tempVal;
    M7::ParamAccessor pa{&tempVal, 0};
    ...
    if (ImGuiKnobs::Knob(label, &tempVal, 0, 1, defaultParamVal, centerParamVal,
                         ImGuiKnobs::ModInfo{}, gNormalKnobSpeed,
                         gSlowKnobSpeed, nullptr, ImGuiKnobVariant_WiperOnly, 0,
                         ImGuiKnobFlags_CustomInput, 10, &conv, this)) {
      GetEffectX()->setParameterAutomated(paramID, M7::math::clamp01(tempVal));
    }
  }
  ...
  void Maj7ImGuiParamFrequencyWithCenter(VstInt32 paramID, VstInt32 ktParamID,
                                         const char *label,
    ...
    Maj7FrequencyConverter conv{cfg, ktParamID};
    if (ImGuiKnobs::Knob(label, &tempVal, 0, 1, default01, centerVal01, modInfo,
                         gNormalKnobSpeed, gSlowKnobSpeed, nullptr,
                         ImGuiKnobVariant_WiperOnly, 0,
                         ImGuiKnobFlags_CustomInput, 10, &conv, this)) {
      GetEffectX()->setParameterAutomated(paramID, M7::math::clamp01(tempVal));
  ...
  void Maj7ImGuiParamMidiNote(VstInt32 paramID, const char *label,
                              int defaultVal, int centerVal) {
    M7::real_t tempVal = 0;
    M7::ParamAccessor pa{&tempVal, 0};
    ...
    if (ImGuiKnobs::Knob(label, &tempVal, 0, 1, defaultParamVal, centerParamVal,
                         ImGuiKnobs::ModInfo{}, gNormalKnobSpeed,
                         gSlowKnobSpeed, nullptr, ImGuiKnobVariant_ProgressBar,
                         0, ImGuiKnobFlags_CustomInput, 10, &conv, this)) {
      GetEffectX()->setParameterAutomated(paramID, M7::math::clamp01(tempVal));
    }
  }

  template <typename Tparam>
  void Maj7ImGuiPowCurvedParam(Tparam paramID, const char *label,
    ...
    if (ImGuiKnobs::Knob(label, &tempVal, 0, 1, defaultParamVal, 0, modInfo,
                         gNormalKnobSpeed, gSlowKnobSpeed, nullptr,
                         ImGuiKnobVariant_WiperOnly, 0,
                         ImGuiKnobFlags_CustomInput, 10, &conv, this)) {
      GetEffectX()->setParameterAutomated((VstInt32)paramID, (tempVal));
  ...
  template <typename Tparam>
  void Maj7ImGuiBipolarPowCurvedParam(Tparam paramID, const char *label,
                                      const M7::PowCurvedParamCfg &cfg,
                                      M7::real_t defaultMS,
                                      ImGuiKnobs::ModInfo modInfo) {
    M7::real_t tempVal;
    ...
    if (ImGuiKnobs::Knob(label, &tempVal, 0, 1, defaultParamVal, 0.5f, modInfo,
                         gNormalKnobSpeed, gSlowKnobSpeed, nullptr,
                         ImGuiKnobVariant_WiperOnly, 0,
                         ImGuiKnobFlags_CustomInput, 10, &conv, this)) {
      GetEffectX()->setParameterAutomated((VstInt32)paramID, (tempVal));
  ...
  template <typename Tparam>
  void Maj7ImGuiDivCurvedParam(Tparam paramID, const char *label,
  ...
  // void Maj7ImGuiParamEnvTime(VstInt32 paramID, const char* label, M7::real_t
  // v_defaultScaled, ImGuiKnobs::ModInfo modInfo) { WaveSabreCore::M7::real_t
  // tempVal; 	M7::EnvTimeParam p{ tempVal , v_defaultScaled }; 	float
  // defaultParamVal = p.GetRawParamValue();
  //	p.SetParamValue(GetEffectX()->getParameter((VstInt32)paramID));
  ...
  void Maj7ImGuiParamFloatN11(VstInt32 paramID, const char *label,
                              M7::real_t v_defaultScaled, float size /*=0*/,
                              ImGuiKnobs::ModInfo modInfo) {
    WaveSabreCore::M7::real_t tempVal;
    M7::ParamAccessor pa{&tempVal, 0};
  ...
  void Maj7ImGuiParamFloatN11WithCenter(VstInt32 paramID, const char *label,
                                        M7::real_t v_defaultScaled,
                                        float centerValN11, float size /*=0*/,
                                        ImGuiKnobs::ModInfo modInfo) {
    WaveSabreCore::M7::real_t tempVal;
    M7::ParamAccessor pa{&tempVal, 0};
    pa.SetN11Value(0, v_defaultScaled);
  ...
  void Maj7ImGuiParamFloat01(VstInt32 paramID, const char *label,
                             M7::real_t v_default01, float centerVal01,
                             float size = 0.0f,
                             ImGuiKnobs::ModInfo modInfo = {}) {
  ...
  void Maj7ImGuiParamCurve(VstInt32 paramID, const char *label,
                           M7::real_t v_defaultN11, M7CurveRenderStyle style,
                           ImGuiKnobs::ModInfo modInfo) {
    WaveSabreCore::M7::real_t tempVal;
    M7::ParamAccessor pa{&tempVal, 0};
    ...
    if (ImGuiKnobs::Knob(label, &tempVal, 0, 1, defaultParamVal, 0.5f, modInfo,
                         gNormalKnobSpeed, gSlowKnobSpeed, nullptr,
                         ImGuiKnobVariant_M7Curve, 0, flags, 10, &conv, this)) {
      GetEffectX()->setParameterAutomated(paramID, M7::math::clamp01(tempVal));
    }
  }

  void Maj7ImGuiParamVolume(VstInt32 paramID, const char *label,
  ...
  void Maj7ImGuiEnvelopeGraphic(const char *label, Tenum delayTimeParamID) {
    ...
    float delayWidth =
        M7::math::clamp(GetEffectX()->getParameter(
                            (VstInt32)delayTimeParamID +
    ...
    float holdWidth =
        M7::math::clamp(
            GetEffectX()->getParameter((VstInt32)delayTimeParamID +
    ...
    float sustainLevel = M7::math::clamp(
        GetEffectX()->getParameter((VstInt32)delayTimeParamID +
                                   (int)M7::EnvParamIndexOffsets::SustainLevel),
        0, 1);
    float sustainYOffset = innerHeight * (1.0f - sustainLevel);

    M7::QuickParam attackCurve{
        GetEffectX()->getParameter((VstInt32)delayTimeParamID +
                                   (int)M7::EnvParamIndexOffsets::AttackCurve)};
    // float attackCurveRaw;
    // M7::CurveParam attackCurve{ attackCurveRaw };
    // attackCurve.SetParamValue(GetEffectX()->getParameter((VstInt32)delayTimeParamID
    // + (int)M7::EnvParamIndexOffsets::AttackCurve));

    M7::QuickParam decayCurve{
        GetEffectX()->getParameter((VstInt32)delayTimeParamID +
                                   (int)M7::EnvParamIndexOffsets::DecayCurve)};
    // float decayCurveRaw;
    // M7::CurveParam decayCurve{ decayCurveRaw };
    // decayCurve.SetParamValue(GetEffectX()->getParameter((VstInt32)delayTimeParamID
    // + (int)M7::EnvParamIndexOffsets::DecayCurve));

    M7::QuickParam releaseCurve{GetEffectX()->getParameter(
        (VstInt32)delayTimeParamID +
        (int)M7::EnvParamIndexOffsets::ReleaseCurve)};
    // float releaseCurveRaw;
    // M7::CurveParam releaseCurve{ releaseCurveRaw };
    // releaseCurve.SetParamValue(GetEffectX()->getParameter((VstInt32)delayTimeParamID
    // + (int)M7::EnvParamIndexOffsets::ReleaseCurve));
    ...
    ImVec2 outerBottomRight = {outerTL.x + gMaxWidth + padding * 2,
                               outerTL.y + innerHeight + padding * 2};
    ImVec2 innerBottomLeft = {innerTL.x, innerTL.y + innerHeight};
    ImVec2 innerBottomRight = {innerBottomLeft.x + gMaxWidth,
                               innerBottomLeft.y};
    // dl->AddRectFilled(outerTL, outerBottomRight,
    // ImGui::GetColorU32(ImGuiCol_FrameBg)); dl->AddRect(outerTL,
    // outerBottomRight, ImGui::GetColorU32(ImGuiCol_Frame));
    ImGui::RenderFrame(outerTL, outerBottomRight,
                       ImGui::GetColorU32(ImGuiCol_FrameBg), true, 3.0f);
    // dl->AddRectFilled(innerTL, innerBottomRight,
    ...
    AddCurveToPath(dl, delayEnd,
                   {attackEnd.x - delayEnd.x, attackEnd.y - delayEnd.y}, false,
                   false, attackCurve.GetRawValue(), lineColor, gLineThickness);
    ...
    AddCurveToPath(dl, holdEnd,
                   {decayEnd.x - holdEnd.x, decayEnd.y - holdEnd.y}, true, true,
                   decayCurve.GetRawValue(), lineColor, gLineThickness);
    ...
    AddCurveToPath(dl, sustainEnd,
                   {releaseEnd.x - sustainEnd.x, releaseEnd.y - sustainEnd.y},
                   true, true, releaseCurve.GetRawValue(), lineColor,
                   gLineThickness);

    dl->AddCircleFilled(delayStart, handleRadius,
                        ImGui::GetColorU32(ImGuiCol_PlotLinesHovered),
                        circleSegments);
    dl->AddCircleFilled(delayEnd, handleRadius,
                        ImGui::GetColorU32(ImGuiCol_PlotLinesHovered),
                        circleSegments);
    dl->AddCircleFilled(attackEnd, handleRadius,
                        ImGui::GetColorU32(ImGuiCol_PlotLinesHovered),
                        circleSegments);
    dl->AddCircleFilled(holdEnd, handleRadius,
                        ImGui::GetColorU32(ImGuiCol_PlotLinesHovered),
                        circleSegments);
    dl->AddCircleFilled(decayEnd, handleRadius,
                        ImGui::GetColorU32(ImGuiCol_PlotLinesHovered),
                        circleSegments);
    dl->AddCircleFilled(sustainEnd, handleRadius,
                        ImGui::GetColorU32(ImGuiCol_PlotLinesHovered),
                        circleSegments);
    dl->AddCircleFilled(releaseEnd, handleRadius,
                        ImGui::GetColorU32(ImGuiCol_PlotLinesHovered),
                        circleSegments);
    ...
    // ImDrawFlags_None, gThickness);

    ImGui::Dummy({outerBottomRight.x - originalPos.x,
                  outerBottomRight.y - originalPos.y});
  }

  virtual bool onWheel(float distance) { return false; }
  virtual bool setKnobMode(VstInt32 val) { return false; }

  // these are called in response to effEditKeyDown/Up, and according to some
  // online sources they are not well/consistently supported, maybe even
  // mac-specific, and shouldn't be used. we will instead use native processing.
  virtual bool onKeyDown(VstKeyCode &keyCode) override {
    return false; // important to return false, so the host doesn't swallow the
                  // message.
  }
  virtual bool onKeyUp(VstKeyCode &keyCode) override {
    //	if (keyCode.character > 0) {
    //		ImGui::GetIO().AddInputCharacterUTF16((unsigned
    // short)keyCode.character); 		return true;
    //	}
    return false; // important to return false, so the host doesn't swallow the
                  // message.
  }
...
protected:
  ...
  VstPlug *GetEffectX() { return static_cast<VstPlug *>(this->effect); }
  ...
  struct GuiContextRestorer {
    ...
    ~GuiContextRestorer() {
      ...
      if (!pushed) {
        // cc::log("popping context; ignoring because it was not pushed.
        // mPrevContext=%p, currentcontext=%p", mPrevContext, current);
        return;
      }
      if (current == mPrevContext) {
        // cc::log("popping context; no need to change context because it's
        // already correct. mPrevContext=%p, currentcontext=%p", mPrevContext,
        // current);
        return;
      }
      // cc::log("popping context; currentcontext=%p  ==>  mPrevContext=%p",
      // current, mPrevContext);
  ...
  static LRESULT WINAPI gWndProc(HWND hWnd, UINT msg, WPARAM wParam,
                                 LPARAM lParam);
