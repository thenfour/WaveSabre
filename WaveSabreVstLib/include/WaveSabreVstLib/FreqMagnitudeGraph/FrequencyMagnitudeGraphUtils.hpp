...
template<int TWidth, int THeight, bool TShowGridLabels> class FrequencyMagnitudeGraph;
...
public:
  ...
  virtual bool InfluencesAutoScale() const { return false; }
  virtual void GetDataBounds(float& minDB, float& maxDB) const { minDB = maxDB = 0; }
  
  // Interaction support
  virtual bool HandleMouse(const FrequencyMagnitudeCoordinateSystem& coords, const ImRect& bb) { return false; }
};
...
struct ScreenSpaceFrequencySampler {
  ...
  static void GenerateFrequencyPoints(int segmentCount, std::vector<float>& frequencies, std::vector<float>& screenX, const FrequencyMagnitudeCoordinateSystem& coords, const ImRect& bb) {
...
struct GridConfiguration {
  ...
  std::vector<float> minorFreqTicks{
    50.0f, 60, 70, 80, 90,
