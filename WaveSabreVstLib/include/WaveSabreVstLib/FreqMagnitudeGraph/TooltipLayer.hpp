...
private:
  ...
  const IEQResponseLayer* mEQLayer = nullptr;
...
public:
  ...
  void SetEQLayer(const EQResponseLayer<TSegmentCount, TFilterCount, TParamCount>* eqLayer)
  {
    ...
    {
      mEvalAtX =
          [eqLayer](float mx, const FrequencyMagnitudeCoordinateSystem& coords, const ImRect& bb, float& outDB) -> bool
      {
  ...
  void Render(const FrequencyMagnitudeCoordinateSystem& coords, const ImRect& bb, ImDrawList* dl) override
  {
    ...
    {
      ImVec2 clampedMouse = {M7::math::clamp(mousePos.x, bb.Min.x, bb.Max.x),
      ...
      bool hasCurve = HasCurve();

      mEvalAtX(clampedMouse.x, coords, bb, magDBLerp);
      ...
      else
      {
        ...
        ImVec2 tipOffset = {8.0f, -8.0f};
        ImVec2 tipAnchor = {clampedMouse.x + tipOffset.x, clampedMouse.y + tipOffset.y};
        ImGui::SetNextWindowPos(tipAnchor, ImGuiCond_Always, ImVec2(0.0f, 1.0f));
        ImGui::BeginTooltip();
        ImGui::Text("%s", freqText);
        ImGui::EndTooltip();
