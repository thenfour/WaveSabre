...
public:
	...
	virtual const char* GetJSONTagName() { return "Maj7Space"; }

	virtual VstInt32 getChunk(void** data, bool isPreset) override
	{
		MAJ7SPACE_PARAM_VST_NAMES(paramNames);
		return GetSimpleJSONVstChunk(GetJSONTagName(), data, GetMaj7Space()->mParamCache, paramNames, [](clarinoid::JsonVariantWriter&) {});
	}

	virtual VstInt32 setChunk(void* data, VstInt32 byteSize, bool isPreset) override
	{
		MAJ7SPACE_PARAM_VST_NAMES(paramNames);
		return SetSimpleJSONVstChunk(GetMaj7Space(), GetJSONTagName(), data, byteSize, GetMaj7Space()->mParamCache, paramNames, [](clarinoid::JsonVariantReader&) {});
	}
